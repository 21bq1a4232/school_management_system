...
def view_fee_details(request, student_id):
    ...
    try:
        ...
        course_fee = FeesStructure.objects.get(course=student.course_id, session_year=student.session_year_id)
        total_fees = course_fee.total_fee  # Access the total_fee from FeesStructure
    except StudentFees.DoesNotExist:
        student_fees = None
        paid_fees = 0
        course_fee = FeesStructure.objects.filter(course=student.course_id, session_year=student.session_year_id).first()
        total_fees = course_fee.total_fee if course_fee else student.total_fees  # Fallback to student's default fees if no FeesStructure exists
